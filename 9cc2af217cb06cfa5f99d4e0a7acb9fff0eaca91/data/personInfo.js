/* ============================================================ */
/* ===                 Person Info Storage                  === */
/* ============================================================ */
function personInfoStorage(){	
	//collapse level 3
	const kesby = {	
		'roseHadkiss': {
			'name':		'Rose Hadkiss',
			'bornName':	'Rosemary Kesby',
			'dates': 	'1925 - 2020',	 
			'about': { 
				'bornOn': '22nd September 1925',
				'diedOn': '28th March 2020',
				'marriedOn':	'26 Mar 1949',
				'marriedAt':	'St Peter and Paul Church, Swanscombe',
				'marriedTo':	'ronaldHadkiss',				
			},	
			'imgIcon': true,
		},
		'jackKesby': {
			'name':		'Jack Kesby',
			'bornName':	'Neville John Kesby',
			'dates': 	'1932 - ', 	
		},	
		
		
		// ----- Bobby ----- //	
		//gen2		
		'maryBobby': {
			'name':		'Mary Kesby',
			'bornName':	'Mary Bobby',
			'dates': 	'1905 - 1969', 
		},
		
		'lotBobby': {
				'name':		'Lot Coyle',
				'bornName':	'Charlotte Bobby',
				'dates': 	'1906 - 1990', 
			},
		'floBobby': {
				'name':		'Flo Bobby',
				'bornName':	'Florence Bobby',
				'dates': 	'1901 - 1965', 
			},
		'charlesEdwardBobby': {
				'name':		'Charles Edward Bobby',
				'dates': 	'1904 - 1980', 
			},
		'sidBobby': {
				'name':		'Sid Bobby',
				'bornName':	'Sidney Grimwood Bobby',
				'dates': 	'1911 -', 
			},
		'harryBobby': {
				'name':		'Harry Bobby',
				'bornName':	'Henry Bobby',
				'dates': 	'1914 - 1984', 
			},
		'jackBobby': {
				'name':		'Jack Bobby',
				'bornName':	'John Bobby',
				'dates': 	'1914 -', 
			},
		'jeanBobby': {
				'name':		'Jean Bobby',
				'bornName':	'Margret Jean Bobby',
				'dates': 	'1921 -', 
			},
			
			//gen3
			'adaBerriman': {
				'name':		'Gran Bobby',
				'bornName':	'Ada Maria Berriman',
				'dates': 	'1883 - 1948',	
			},
				//gen4
				'mariaBerriman': {
					'name':		'Maria',
					'dates': 	'1852 - ', 
				},
				'johnBerriman': {
					'name':		'John Berriman',
					'dates': 	'1850 - ',	
				},
					//gen5
					'georgeBerriman': {
						'name':		'George Berriman',
						'dates': 	'1814 - ',	
					},
					'anneBerriman': {
						'name':		'Anne',
						'dates': 	'1823 - ',	
					},
					
			//gen3
			'sidneyCharlesBobby': {
				'name':		'Sidney Charles Bobby',
				'dates': 	'1877 - 1957', 	
			},
			
			'florencePrimroseBobby': {
				'name':		'Florence Primrose Bobby',
				'dates': 	'1895 - 1895', 	
			},
			'margaretEllenBobby': {
				'name':		'Margaret Ellen Bobby',
				'dates': 	'1894 - 1972', 	
			},
			'charlotteBobby': {
				'name':		'Charlotte Bobby',
				'dates': 	'1890 - 1974', 	
				'fullDates': '',
			},
			'samuelJamesBobby': {
				'name':		'Samuel James Bobby',
				'dates': 	'1886 - 1957', 	
			},
			'annieFlorenceBobby': {
				'name':		'Annie Florence Bobby',
				'dates': 	'1886 - 1900', 	
			},
			'maryAnnBobby': {
				'name':		'Mary Ann Bobby',
				'dates': 	'1882 - 1937', 	
			},
			'janeClarkBobby': {
				'name':		'Jane Clark Bobby',
				'dates': 	'1881 - 1905', 	
			},
			'jamesHerbertBobby': {
					'name':		'James Herbert Bobby',
					'dates': 	'1879 - 1884', 	
				},
		
					//gen4
					'fannyClarkChapman': {
						'name':		'Fanny Clark Bobby',
						'bornName':	'Fanny Clark Chapman',
						'dates': 	'1857 - 1926', 	
					},			
					'benjaminBobby': {
						'name':		'Benjamin Bobby',
						'dates': 	'1856 - 1926', 	
					},
					
					'nathonBobby': {
						'name':		'Nathon Bobby',
						'dates': 	'1852 - 1906', 	
					},
					'annBobby': {
						'name':		'Ann Bobby',
						'dates': 	'1849 - 1871', 	
					},
					'carolineBobby': {
						'name':		'Caroline Bobby',
						'dates': 	'1846 - 1870', 	
					},
					'harrieBobby': {
						'name':		'Harrie Bobby',
						'dates': 	'1841 - 1861', 	
					},	
					'walterBobby': {
						'name':		'Walter Bobby',
						'dates': 	'1839 - 1886', 	
					},	
					'williamBobby': {
						'name':		'William Bobby',
						'dates': 	'1835 - 1882', 	
					},
		
						//gen5
						'charlotteAtkins': {
							'name':		'Charlotte Atkins',
							'dates': 	'1816 - 1900', 	
						},
						'william5Bobby': {
							'name':		'William Bobby',
							'LI_name':  'William5 Bobby',
							'dates': 	'1814 - 1868', 	
						},							
						
						'caroline5Bobby': {
							'name':		'Caroline',
							'dates': 	'- 1826', 	
						},
						'suzannahBobby': {
							'name':		'Suzannah',
							'dates': 	'1827 -', 
						},
						'sarahBobby': {
							'name':		'Sarah',
							'dates': 	'1827 -', 	
						},
						'benjamin5Bobby': {
							'name':		'Benjamin',
							'dates': 	'1824 -', 	
						},
						'estherBobby': {
							'name':		'Esther Bobby',
							'dates': 	'1821 - 1881', 	
						},
		
							//gen6
							'benjamin6Bobby': {
								'name':		'Benjamin Bobby',
								'LI_name':	'Benjamin6 Bobby',
								'dates': 	'1782 - 1857', 	
							},
							'sarahHubbard': {
								'name':		'Sarah Hubbard',
								'dates': 	'1784 - 1845', 
							},
							
							'amyBobby': {
								'name':		'Amy',
								'dates': 	'1801 - 1841', 	
							},
							'susanBobby': {
									'name':		'Susan Bobby',
									'dates': 	'1784 - 1853', 
								},
								
								//gen7
								'jamesBobby': {
									'name':		'James Bobby',
									'dates': 	'1761 - 1823', 	
								},
								'suzannaBobby': {
									'name':		'Suzanna',
									'dates': 	'1758 - 1821', 	
								},
								
								'thomasBobby': {
									'name':		'Thomas Bobby',
									'dates': 	'1759 - 1816', 	
								},
								'john7Bobby': {
									'name':		'John Bobby',
									'dates': 	'1756 - 1764', 	
								},
								'william7Bobby': {
									'name':		'William Bobby',
									'LI_name':	'William7 Bobby',
									'dates': 	'1763 - 1841', 	
								},
									
									//gen8
									'benjamin8Bobby': {
										'name':		'Benjamin Bobby',
										'LI_name':	'Benjamin8 Bobby',
										'dates': 	'1729 - 1764', 	
									},
									'ursulaPitman': {
										'name':		'Ursula Pitman',
										'dates': 	'1727 - 1781', 	
									},
	
									'sarahBobby': {
										'name':		'Sarah Bobby',
										'dates': 	'1730 - 1810', 	
									},									
									'mary8Bobby': {
										'name':		'Mary Bobby',
										'LI_name':	'Mary8Bobby',
										'dates': 	'1736 - ', 	
									},									
									'bridgetBobby': {
										'name':		'Bridget Bobby',
										'dates': 	'1737 - 1815', 	
									},									
									'ellisBobby': {
										'name':		'Ellis Bobby',
										'dates': 	'1736 - 1812', 	
									},
									
										//gen9
										'johnBobby': {
											'name':		'John Bobby',
											'dates': 	'1703 - 1771', 	
										},
										'maryHayward': {
											'name':		'Mary Hayward',
											'dates': 	'1704 - 1759', 	
										},
										
										'marthaPrime': {
											'name':		'Martha Prime',
											'dates': 	' - ', 	
										},	
										
										'susan9Bobby': {
											'name':		'Susan',
											'LI_name':	'Susan9 Bobby',
											'dates': 	'1699 - 1762', 	
										},	
										'dorothyBobby': {
											'name':		'Dorothy',
											'dates': 	'1697 - 1759', 	
										},	
										'dorothy9Bobby2': {
											'name':		'Dorothy',
											'LI_name':	'Dorothy9 Bobby 2',
											'dates': 	'1695 - 1695', 	
										},	
										'hannahBobby': {
											'name':		'Hannah',
											'dates': 	'1692 - 1757', 	
										},	
										'mary9Bobby': {
											'name':		'Mary',
											'LI_name':	'Mary9 Bobby',
											'dates': 	'1689 - 1742', 	
										},	
										'elizabethBobby': {
											'name':		'Elizabeth ',
											'dates': 	'1686 - 1732', 	
										},	
										
											//gen10
											'johnBauby': {
												'name':		'John Bauby',
												'dates': 	'1657 - 1720', 	
											},
											'elizabethFord': {
												'name':		'Elizabeth Ford',
												'dates': 	'1656 - 1717', 	
											},
											
											'elizabethBauby': {
												'name':		'Elizabeth',
												'dates': 	'1651 - 1721', 	
											},
											'thomasBauby': {
												'name':		'Thomas',
												'dates': 	'1659 - ', 	
											},
											
												//gen11
												'john11Bauby': {
													'name':		'John Bauby',
													'LI_name':	'John11 Bauby',
													'dates': 	'1626 - 1659', 	
												},
												'mary11Bauby': {
													'name':		'Mary',
													'LI_name':	'Mary11 Bauby',
													'dates': 	'1622 - 1660', 	
												},
												
												'edmondBauby': {
													'name':		'Edmond',
													'dates': 	'1622 - 1671', 	
												},
												'annBauby': {
													'name':		'Ann',
													'dates': 	'1624 - 1684', 	
												},
												'mary11Bauby_sib': {
													'name':		'Mary',
													'LI_name':	'Mary11 Bauby2',
													'dates': 	'1626 - 1632', 	
												},
												'susan11Bauby': {
													'name':		'Susan',
													'LI_name':	'Susan11 Bauby',
													'dates': 	'1629 - 1681', 	
												},
											
												//gen12
												'johnBolbi': {
													'name':		'John Bolbi',
													'dates': 	'1581 - 1657', 	
												},
												'maryBolbi': {
													'name':		'Mary',
													'dates': 	'1584 - 1660', 	
												},
												
												'faithBolbi': {
													'name':		'Faith',
													'dates': 	'1581 - 1610', 	
												},
												'williamBolbi': {
													'name':		'William',
													'LI_name':	'William Bolbi',
													'dates': 	'1582 - 1582', 	
												},
												'william12Bolbi2': {
													'name':		'William Bolbi',
													'LI_name':	'William12 Bolbi2',
													'dates': 	'1583 - 1640', 	
												},
												'thomasBolbi': {
													'name':		'Thomas',
													'LI_name':	'Thomas Bolbi',
													'dates': 	'1584 - 1666', 	
												},
												'mary12Bolbi': {
													'name':		'Mary',
													'LI_name':	'Mary12 Bolbi',
													'dates': 	' - ', 	
												},
												'margaretBolbi': {
													'name':		'Margaret',
													'LI_name':	'Margaret Bolbi',
													'dates': 	'1587 - 1669', 	
												},
												'edmondBolbi': {
													'name':		'Edmond',
													'LI_name':	'Edmond Bolbi',
													'dates': 	'1589 - 1672', 	
												},
												'edwardBobli': {
													'name':		'Edward',
													'LI_name':	'Edward Bolbi',
													'dates': 	'1591 - 1676', 	
												},
												'aliceBolbi': {
													'name':		'Alice',
													'LI_name':	'Alice Bolbi',
													'dates': 	'1594 - 1646', 	
												},
												
													//gen13
													'edmundBawbeye': {
														'name':		'Edmund Bawbeye',
														'dates': 	'1553 - 1608', 	
													},
													'margretMaen': {
														'name':		'Margret Maen',
														'dates': 	'1554 - 1596', 	
													},
													
													'sonBawbeye': {
														'name':		'Son',
														'LI_name':	'Son Bawbeye',
														'dates': 	'1544 - 1544', 	
													},
													'daughterBawbeye': {
														'name':		'Daughter',
														'LI_name':	'Daughter Bawbeye',
														'dates': 	'1556 - 1556', 	
													},
													'johnBawbeye': {
														'name':		'John Bawbeye',
														'dates': 	'1546 - 1546', 	
													},
													'alynBawbeye': {
														'name':		'Alyn Bawbeye',
														'dates': 	'1551 - 1551', 	
													},
													'rogerBawbeye': {
														'name':		'Roger Bawbeye',
														'dates': 	'1555 - 1555', 	
													},
													'robert13Bawbeye': {
														'name':		'Robert Bawbeye',
														'LI_name':	'Robert13 Bawbeye',
														'dates': 	'1559 - 1601', 	
													},
													'alline13Bawbeye': {
														'name':		'Alline Bawbeye',
														'LI_name':	'Alline13 Bawbeye',
														'dates': 	'1560 - 1630', 	
													},
													'richardBawbeye': {
														'name':		'Richard Bawbeye',
														'dates': 	'1596 - 1641', 	
													},
													'susanBawbeye': {
														'name':		'Susan Bawbeye',
														'dates': 	'1598 - 1598', 	
													},
													'susan13Bawbeye2': {
														'name':		'Susan Bawbeye',
														'LI_name':	'Susan13 Bawbeye2',
														'dates': 	'1600 - ', 	
													},
													'john13Bawbeye2': {
														'name':		'John Bawbeye',
														'LI_name':	'John13 Bawbeye2',
														'dates': 	'1604 - ', 	
													},
														
														//gen14
														'robertBawbeye': {
															'name':		'Robert Bawbeye',
															'dates': 	'1525 - 1610', 	
														},
														'allineBawbeye': {
															'name':		'Alline',
															'LI_name':	'Alline Bawbeye',
															'dates': 	'1526 - 1582', 	
														},
														'mary14Bawbeye': {
															'name':		'Mary',
															'LI_name':	'Mary14 Bawbeye',
															'dates': 	' - ', 	
														},
													
		// ----- Kesby ----- //	
		//gen2
		'johnNevilleKesby': {
			'name':		'John Neville Kesby',
			'dates': 	'1900 - 1997', 
		},
		
		'williamKesby': {
			'name':		'William Kesby',
			'dates': 	'1901 - ', 
		},
		'beatriceHarrietKesby': {
			'name':		'Beatrice Harriet Kesby',
		},
		'nellKesby': {
			'name':		'Nell Kesby',
			'bornName':	'Rose Ellen Kesby',
			'dates': 	'1906 - 2008', 
		},
		'charlesKesby': {
			'name':		'Charles Kesby',
			'dates': 	' - 1949', 
		},
		'alfredKesby': {
			'name':		'Alfred Kesby',
		},
	
			//gen3
			'emmaMariaNeville': {
				'name':		'Emma Maria Kesby',
				'bornName':	'Emma Maria Neville',
				'dates': 	'1875 - ', 
			},
			
			'williamNeville': {
				'name':		'William Neville',
				'dates': 	'1878 -', 
			},			
			'percyNeville': {
				'name':		'Percy Neville',
				'dates': 	'1880 -', 
			},			
			'beatriceNeville': {
				'name':		'Beatrice Neville',
				'dates': 	'1885 -', 
			},			
			'bernardNeville': {
				'name':		'Bernard Neville',
				'dates': 	'1887 -', 
			},
			
				//gen4
				'estherNeville': {
					'name':		'Esther',
				},
				'johnNeville': {
					'name':		'John Neville',
				},
					
					//gen5
					'unknownNeville': {
						'name':		'-',
						'LI_name':		'Unknown Neville',
					},
					'harrietNeville': {
						'name':		'Harriet',
					},
			
			//gen3
			'alfredSmithKesby': {
				'name':		'Alfred Smith Kesby',
				'dates': 	'1877 - ', 
			},
			
			'charlesSmith': {
				'name':		'Charles Smith',
				'dates': 	'1856 - ', 
			},			
			'williamJamesSmith': {
				'name':		'William James Smith',
				'dates': 	'1874 - ', 
			},
			
				//gen4
				'johnKesby': {
					'name':		'John Kesby',
					'dates': 	'1852 - ', 
				},	
				'harrietSmith': {
					'name':		'Harriet Smith',
					'dates': 	'1835 - ', 
				},		
		
				'benjaminSmith': {
					'name':		'Benjamin Smith',
					'dates': 	'1830 - ', 
				},	
				
	
	}
	
	
	const hadkiss = {
		'ronaldHadkiss': {
			'name':		'Ron Hadkiss',
			'bornName':	'Ronald Leonard Hadkiss',
			'dates': 	'1925 - 1995',	 
			'about': { 
				'bornOn': 	'6th August 1925',
				'bornAt': 	'34 Enfield Rd, Dalston, London, N1',
				'diedOn':	'9th January 1995',
				'diedAt':	'West Hill Hospital, Dartford',	
				'marriedOn':	'26 Mar 1949',
				'marriedAt':	'St Peter and Paul Church, Swanscombe',
				'marriedTo':	'Rose Hadkiss',				
			},	
			//'imgIcon': true,			
		},	
		'williamHadkiss': {
			'name':		'William Hadkiss',
			'dates': 	'1922 - 1992', 
		},
		'ivyHadkiss': {
			'name':		'Ivy Frances Hadkiss',
			'dates': 	'1915 - 1919', 
		},
		'ireneHadkiss': {
			'name':		'Irene Ellen Hadkiss',
			'dates': 	'1920 - ', 
		},
		
		// ----- hadkiss ----- //	
		//gen2	
		'billHadkiss': {
			'name':		'Bill Hadkiss',
			'bornName':	'William Hadkiss',
			'dates': 	'1888 - 1968', 
		},
		
		'nellHadkiss': {
			'name':		'Nell Hadkiss',
			'bornName':	'Ellen Hadkiss',
			'dates': 	'1890 -', 
		},
		'maryAnnHadkiss': {
			'name':		'Mary Anne Hadkiss',
			'dates': 	'1893 - 1915', 
		},
		'samuel2Hadkiss': {
			'name':		'Samuel Hadkiss',
			'dates': 	'1895 - 1916', 
		},
		'leonardGeorgeHadkiss': {
			'name':		'Leonard George Hadkiss',
			'dates': 	'1901 - 1901', 
		},
		'elizabethEstherHadkiss': {
			'name':		'Elizabeth Esther Hadkiss',
			'dates': 	'1902 - 1903', 
		},
		
			//gen3	
			'samuelHadkiss': {
				'name':		'Samuel Hadkiss',
				'dates': 	'1857 - 1922', 
			},
			
			'annHadkiss': {
				'name':		'Ann Hadkiss',
				'dates': 	'1853 - ', 
			},
			'william3Hadkiss': {
				'name':		'William Hadkiss',
				'dates': 	'1855 - ', 
			},
			'johnHadkiss': {
				'name':		'John Hadkiss',
				'dates': 	'1859 - ', 
			},
			'thomasHadkiss': {
				'name':		'Thomas Hadkiss',
				'dates': 	'1862 - ', 
			},
			'charlesHadkiss': {
				'name':		'Charles Hadkiss',
				'dates': 	'1864 - ', 
			},
			'juliaHadkiss': {
				'name':		'Julia Hadkiss',
				'dates': 	'1870 - ', 
			},
			
				//gen4	
				'william4Hadkiss': {
					'name':		'William Hadkiss',
					'dates': 	'1830 - ', 
				},
					//gen5	
					'thomas5Hadkiss': {
						'name':		'Thomas Hadkiss',
						'dates': 	'1791 - ', 
					},	
					'unknown5Hadkiss1': {
						'name':		'',
						'dates': 	'', 
					},
					
				//gen4	
				'juliaMitchell': {
					'name':		'Julia Mitchell',
					'dates': 	'1831 - ', 
				},
			
			//gen3	
			'elizabethCoombs': {
				'name':		'Elizabeth Esther Coombs',
				'dates': 	'1864 - 1903', 
			},
			
			'ellenCoombs': {
				'name':		'Ellen Coombs',
				'dates': 	'1871 - ', 
			},
			'maryCoombs': {
				'name':		'Mary Coombs',
				'dates': 	'1869 - ', 
			},
			'stephenCoombs': {
				'name':		'Stephen Coombs',
				'dates': 	'1863 - ', 
			},
			'roberthaCoombs': {
				'name':		'Robertha',
				'dates': 	'1863 - ', 
			},
		'stephen2Coombs': {
			'name':		'Stephen Coombs',
			'dates': 	'1886 - ', 
		},
		'robertCoombs': {
			'name':		'Robert Coombs',
			'dates': 	'1890 - ', 
		},
		'henryCoombs': {
			'name':		'Henry Coombs',
			'dates': 	'1892 - ', 
		},
		'williamCoombs': {
			'name':		'William Coombs',
			'dates': 	'1893 - ', 
		},
		'adaCoombs': {
			'name':		'Ada Coombs',
			'dates': 	'1896 - ', 
		},
		'hectorCoombs': {
			'name':		'Hector Coombs',
			'dates': 	'1901 - ', 
		},
					
				//gen4	
				'stephenGeorgeCoombs': {
					'name':		'Stephen George Coombs',
					'dates': 	'1826 - ', 
				},
				'mariaCoombs': {
					'name':		'Maria',
					'dates': 	'1838 - ', 
				},
		
		
		
		// ----- claydon ----- //	
		//gen2	
		'berthaClaydon': {
			'name':		'Bertha Isabella Claydon',
			'dates': 	'1896 - 1973', 
		},
		
		'leonardClaydon': {
			'name':		'Leonard Claydon',
			'dates': 	'1903 - ', 
		},
		'elsieClaydon': {
			'name':		'Elsie Claydon',
			'dates': 	'1907 - ', 
		},
		'williamErnestClaydon': {
			'name':		'William Ernest Claydon',
			'dates': 	'1898 - 1971', 
		},
		'georgeClaydon': {
			'name':		'George Claydon',
			'dates': 	'1892 - ', 
		},
		'adaClaydon': {
			'name':		'Ada Claydon',
			'dates': 	'1894 - ', 
		},
		
			//gen3	
			'georgeOliverClaydon': {
				'name':		'George Oliver Claydon',
				'dates': 	'1864 - 1911', 
			},
			
			'florenceClaydon': {
				'name':		'Florence Claydon',
				'dates': 	'1874 - ', 
			},
			'majorClaydon': {
				'name':		'Major A Claydon',
				'dates': 	'1871 - ', 
			},
			'ernestEHClaydon': {
				'name':		'Ernest E H Claydon',
				'dates': 	'1868 - ', 
			},
			'aliceClaydon': {
				'name':		'Alice L Claydon',
				'dates': 	'1866 - ', 
			},
			'robertClaydon': {
				'name':		'Robert L Claydon',
				'dates': 	'1865 - ', 
			},
			'harryClaydon': {
				'name':		'Harry E Claydon',
				'dates': 	'1863 - ', 
			},
			'arthurClaydon': {
				'name':		'Arthur Claydon',
				'dates': 	'1857 - ', 
			},
			'joshuaClaydon': {
				'name':		'Joshua J Claydon',
				'dates': 	'1856 - ', 
			},
			'maryClaydon': {
				'name':		'Mary Claydon',
				'dates': 	'1854 - ', 
			},
			'fredrickClaydon': {
				'name':		'Fredrick Claydon',
				'dates': 	'1851 - ', 
			},
			'arabellaClaydon': {
				'name':		'Arabella Claydon',
				'dates': 	'1848 - ', 
			},
		
			
				//gen4	
				'thomasClaydon': {
					'name':		'Thomas Claydon',
					'dates': 	'1823 - ', 
				},
			
					//gen5	
					'mary5Claydon': {
						'name':		'Mary',
						'dates': 	'', 
					},
					'unknown5Claydon': {
						'name':		'',
						'dates': 	'', 
					},
		
				//gen4	
				'maryRawlinson': {
					'name':		'Mary Rawlinson',
					'dates': 	'1829 - ', 
				},
					//gen5	
					'elizabethRawlinson': {
						'name':		'Elizabeth',
						'dates': 	'', 
					},
					'unknownRawlinson': {
						'name':		'',
						'dates': 	'', 
					},
					
			//gen3	
			'fannyAtkinson': {
				'name':		'Fanny Atkinson',
				'bornName':	'Frances Atkinson',
				'dates': 	'1866 - ', 
			},
			
			'kateAtkinson': {
				'name':		'Kate Atkinson',
				'dates': 	'1872 - ', 
			},
			'jamesRAtkinson': {
				'name':		'James R Atkinson',
				'dates': 	'1874 - ', 
			},
			'helenaAtkinson': {
				'name':		'Helena (Elenor) Atkinson',
				'dates': 	'1877 - ', 
			},
			'annieAtkinson': {
				'name':		'Annie Atkinson',
				'dates': 	'1879 - ', 
			},
			'adaAtkinson': {
				'name':		'Ada Atkinson',
				'dates': 	'1881 - ', 
			},
			'florenceAtkinson': {
				'name':		'Florence Atkinson',
				'dates': 	'1883 - ', 
			},
			'georgeAtkinson': {
				'name':		'George Atkinson',
				'dates': 	'1885 - ', 
			},
			'gertrudeAtkinson': {
				'name':		'Gertrude Atkinson',
				'dates': 	'1887 - ', 
			},
			'sidneyAtkinson': {
				'name':		'Sidney Atkinson',
				'dates': 	'1889 - ', 
			},	
			
				//gen4	
				'jamesAtkinson': {
					'name':		'James Atkinson',
					'dates': 	'1842 - ', 
				},
				'rachelAtkinson': {
					'name':		'Rachel',
					'dates': 	'1852 - ', 
				},
	};
	
	const peal = {	
	};
	const mckenzie = {	
	};
	
	
	var allFams = {'kesby': kesby, 'hadkiss': hadkiss, 'peal': peal, 'mckenzie': mckenzie};

	/*
	switch (famName){
		case 'kesby':
			return kesbyInfo;
		break;
		case 'hadkiss':
			return hadkissInfo;
		break;
		case 'peal':
			return pealInfo;
		break;
		case 'mckenzie':
			return mckenzieInfo;
		break;
	}*/
	return allFams;
}

/* ------------------------------------------------ */